...
def per_epoch(model: nn.Module, dl: torch.utils.data.DataLoader, optimizer: torch.optim.Optimizer, loss_fn: nn.Module, device: str, train: bool = True):
    ...
    for idx, batch in iters:
        ...
        iters.set_description(f"[{mode}] loss: {_loss.detach().cpu().item(): .3f} avgLoss: {loss/(idx+1)}")
    return loss/len(dl)
...
def execute(cfg: Dict, device: str):
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer, T_max=len(train_dl)*cfg['train']['epochs'], eta_min=cfg['train']['scheduler']['eta_min']
        )
    ...
    for epoch in range(cfg['train']['epochs']):
        ...
        print(f"Epoch Summary: [{epoch+1}] train_loss: {train_loss: .3f}, val_loss: {val_loss: .3f}, lr: {scheduler.get_lr()}")
